...
class MultiShipEnv(object):
    ...
    first_step = True
    last_distance_to_goal = np.zeros(3)
    last_formation_distance = np.zeros(3)
    ...
    def _get_state_and_reward(self):
        ...
        if self.first_step:
            ...
            self.last_distance_to_goal[0] = dg1
            self.last_distance_to_goal[1] = dg2
            self.last_distance_to_goal[2] = dg3
            self.last_formation_distance[0] = d12
            self.last_formation_distance[1] = d13
            self.last_formation_distance[2] = d23

        # reward of distance to goal
        factor_goal = 0.02
        rg1, rg2, rg3 = 0., 0., 0.
        if self.last_distance_to_goal[0] - dg1 > 0:
            rg1 += factor_goal
        else:
            rg1 -= factor_goal
        if self.last_distance_to_goal[1] - dg2 > 0:
            rg2 += factor_goal
        else:
            rg2 -= factor_goal
        if self.last_distance_to_goal[2] - dg3 > 0:
            rg3 += factor_goal
        else:
            rg3 -= factor_goal
        ...
        # reward of successful formation
        factor_formation = 0.0
        rf1, rf2, rf3= 0., 0., 0.
        ...
        # reward of distance change of formation
        factor_formation_distance = 0.005
        if abs(self.last_formation_distance[0] - self.formation_distance) - error_d12 > 0:
            rf1 += factor_formation_distance
            rf2 += factor_formation_distance
        else:
            rf1 -= factor_formation_distance
            rf2 -= factor_formation_distance
        if abs(self.last_formation_distance[1] - self.formation_distance) - error_d13 > 0:
            rf1 += factor_formation_distance
            rf3 += factor_formation_distance
        else:
            rf1 -= factor_formation_distance
            rf3 -= factor_formation_distance
        if abs(self.last_formation_distance[2] - self.formation_distance) - error_d23 > 0:
            rf2 += factor_formation_distance
            rf3 += factor_formation_distance
        else:
            rf2 -= factor_formation_distance
        ...
        r_action3 = factor_action * (self.action[2, 0] - abs(self.action[2, 1]))
...
class TrainingThread(threading.Thread):
    ...
    def value(self):
        test_episode = 1000
        network_time = '2020-07-08T23-10-20'
