...
class MultiShipEnv(object):
    ...
    def _get_state_and_reward(self):
        ...
        if error_d12 > self.formation_distance_error:
            rf1 += -factor_formation*error_d12
            rf2 += -factor_formation*error_d12
        error_d13 = abs(d13 - self.formation_distance)
        if error_d13 > self.formation_distance_error:
            rf1 += -factor_formation*error_d13
            rf3 += -factor_formation*error_d13
        error_d23 = abs(d23 - self.formation_distance)
        if error_d23 > self.formation_distance_error:
            rf2 += -factor_formation*error_d23
            rf3 += -factor_formation*error_d23
        ...
        # reward of action
        r_action1 = self.action[0, 0] - abs(self.action[0, 1])
        r_action2 = self.action[1, 0] - abs(self.action[1, 1])
        r_action3 = self.action[2, 0] - abs(self.action[2, 1])

        reward[0] = rg1 + rf1 + rc1 + r_action1
        reward[1] = rg2 + rf2 + rc2 + r_action2
        reward[2] = rg3 + rf3 + rc3 + r_action3
        return state, reward
...
class TrainingThread(threading.Thread):
    ...
    def train(self):
        ...
        for i_episode in range(self.MAX_EPISODES):
            ...
            for t in range(self.MAX_EP_STEPS):
                ...
                a = action.numpy()
                # left_cmd1, right_cmd1, left_angle1, right_angle1 = a[0, 0], a[0, 0], a[0, 1]*np.pi/2, a[0, 1]*np.pi/2
                # left_cmd2, right_cmd2, left_angle2, right_angle2 = a[1, 0], a[1, 0], a[1, 1]*np.pi/2, a[1, 1]*np.pi/2
                # left_cmd3, right_cmd3, left_angle3, right_angle3 = a[2, 0], a[2, 0], a[2, 1]*np.pi/2, a[2, 1]*np.pi/2
    ...
    def value(self):
        test_episode = 5000
        for m in range(self.n_agents):
            name = str(test_episode) + '_' + 'agent_' + str(m) + '_'
